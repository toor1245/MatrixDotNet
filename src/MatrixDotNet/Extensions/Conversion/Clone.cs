...
namespace MatrixDotNet.Extensions.Conversion
{
    ...
    public static unsafe partial class MatrixConverter
    {
        ...
        public static void CopyTo<T>(Matrix<T> matrix1, int dimension1, int start, Matrix<T> matrix2, int dimension2,
            ...
            where T : unmanaged
        {
            ...
            {
                var span2 = new Span<T>(ptr2, matrix2.Length);
                var span1 = new Span<T>(ptr1, matrix1.Length);
        ...
        /// <param name="length">length copy.</param>
        /// <typeparam name="T">unmanaged type</typeparam>
        public static void CopyTo<T>(State state, Matrix<T> matrix1, int dimension1, int start, Matrix<T> matrix2,
            int dimension2, int destinationIndex, int length)
            where T : unmanaged
        {
            if (state == State.Row)
                for (int i = start, k = destinationIndex; k < length; i++, k++)
